...
class PlaceListViewController: UIViewController {
    ...
    func saveData(index: Int) {
        ...
        // If we have saved a record, we wil have an ID
        if places[index].placeDocumentID != "" {
            ...
            ref.setData(dataToSave) { (error) in
                if let error = error {
                    ...
                    print("Doc updated with ref ID \(ref)")
                }
            }
        } else { // otherwise we don't have a docusment ID so we need to create a ref ID and save a new doc
